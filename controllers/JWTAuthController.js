...
class JWTAuthController {
    ...
    startCleanupIntervals() {
        ...
        setInterval(() => {
            ...
            for (const [token, data] of this.csrfTokens.entries()) {
                if (now - data.created > 30 * 60 * 1000) { // 30 minutes
    ...
    showLoginPage = async (req, res) => {
        try {
            ...
            const currentLang = req.query.lang || req.language || req.cookies.language || req.cookies.i18next || 'uz';
        ...
        try {
            ...
            if (!validation.valid) {
                ...
                if (isAjax) {
                    ...
                    return res.redirect(this.buildLoginRedirectUrl(req, validation.message));
                }
            }
            ...
            if (!this.validateCSRFToken(req, csrfToken)) {
                ...
                if (isAjax) {
                    return this.sendResponse(res, 403, false, 'Invalid security token. Please refresh the page.', null, {
                        code: 'CSRF_INVALID'
                    });
                } else {
                    return res.redirect(this.buildLoginRedirectUrl(req, 'Invalid security token. Please refresh the page.'));
                }
            }
            ...
            if (this.isIPBlocked(clientIP)) {
                ...
                if (isAjax) {
                    return this.sendResponse(res, 429, false, 'Too many failed login attempts. Please try again later.', null, {
                        code: 'IP_BLOCKED'
                    });
                } else {
                    return res.redirect(this.buildLoginRedirectUrl(req, 'Too many failed login attempts. Please try again later.'));
                }
            }
            ...
            if (!authResult.success) {
                ...
                if (isAjax) {
                    ...
                    return res.redirect(this.buildLoginRedirectUrl(req, authResult.message));
                }
            }
            ...
            const redirectUrl = this.getRedirectUrl(authResult.user, authResult.userType, req.session?.returnTo);
            ...
            return this.sendResponse(res, 500, false, 'An internal error occurred. Please try again.', null, {
                code: 'INTERNAL_ERROR'
            });
        }
    };
    ...
    refreshToken = async (req, res) => {
        try {
            ...
            if (!tokens.refreshToken) {
                return this.sendResponse(res, 401, false, 'Refresh token not found', null, {
                    code: 'REFRESH_TOKEN_MISSING'
                });
            }

            const refreshResult = await TokenService.refreshAccessToken(tokens.refreshToken, { User, Admin });
            
            if (!refreshResult.success) {
                // Clear invalid cookies
                TokenService.clearAuthCookies(res);
                
                return this.sendResponse(res, 401, false, 'Token refresh failed', null, {
                    code: 'REFRESH_FAILED',
    ...
    me = async (req, res) => {
        try {
            ...
            if (req.user.userType === 'admin') {
                userData = await Admin.findById(req.user.userId).select('-password -__v');
            } else {
                userData = await User.findById(req.user.userId).select('-password -__v');
            }
     ...
     */
    validateLoginInput({ email, password, csrfToken }) {
        if (!email) {
            return { valid: false, message: 'Email or identifier is required', field: 'email' };
        }
    ...
    validateCSRFToken(req, token) {
        ...
        if (tokenData.ip !== req.ip || tokenData.userAgent !== req.get('User-Agent')) {
            this.csrfTokens.delete(token);
            return false;
        }
    ...
    isIPBlocked(ip) {
        ...
        const now = Date.now();
        const recentAttempts = attempts.filter(time => now - time < 60 * 60 * 1000); // 1 hour window
    ...
    buildLoginRedirectUrl(req, error) {
        const currentLang = req.language || req.cookies.language || req.cookies.i18next || 'uz';
        let url = '/auth/login';
